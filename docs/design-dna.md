...
Mobile modal panels retain the shared 300ms entrance on their own surface. The portalled navigation uses a 400ms ease entrance (opacity and 28px translation, without scaling). Closing is immediate: remove the panel and background blur together, release the focus trap and scroll lock, and restore focus without an exit animation. Background blur remains immediate on opening. The selected-services icon uses a brief 220ms entrance. Reduced motion disables these reveals. Mobile submenus expand to their natural content height without clipping or long per-item delays.

Doctor directory search and specialty selection are separate modes: typing a name or specialty searches all doctors and resets the category; selecting a category clears the query. Saved links with a nonblank search prioritize that search over a stale category. Clear, reload, profile return links and browser navigation keep the URL and controls in sync.
