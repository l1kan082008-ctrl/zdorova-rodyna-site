...
export async function createPriceWorkbook(items: PriceItem[]) {
  ...
  const rows = items.map(item => [
    item.code ?? "", item.name, item.amount, item.categoryLabel, item.turnaround || "Уточнюйте",
  ...
  const sheet = XLSX.utils.aoa_to_sheet([
    ...
    ["Код", "Назва", "Ціна", "Категорія", "Термін виконання", "CITO", "Активна", "Синоніми", "Порядок", "ID"],
    ...rows,
  ]);
  sheet["!cols"] = [16, 68, 12, 32, 22, 10, 12, 50, 12, 40].map(wch => ({ wch }));
  sheet["!autofilter"] = { ref: `A3:J${rows.length + 3}` };
