...
export function HomeSearch({ items }: { items: HomeSearchItem[] }) {
    ...
    return groupOrder
      .map((kind) => {
        const matches = items
          ...
          .slice(0, normalizedQuery ? 5 : kind === "price" ? 4 : 3);

        return { kind, items: matches };
      })
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const exactResult = allResults.find(
      (item) => normalizeMedicalSearch(item.title) === normalizedQuery,
    );
    const destination = exactResult ?? (allResults.length === 1 ? allResults[0] : null);
      ...
      {isOpen ? (
        ...
        >
          ...
          <p className="sr-only" aria-live="polite">
            {query ? `Знайдено результатів: ${resultCount}` : "Популярні результати"}
          </p>
